<script lang="ts">
  ...
  interface ReadingStatsData {
    stats: {
      ...
      chapterStats: Record<number, { timeSpent: number; readCount: number; lastRead: number }>;
    };
  }
  ...
  export let readingStats: ReadingStatsData | null = null;
  ...
  // 计算连续阅读天数
  function calculateReadingStreak(dailyStats: Record<string, DailyStats>): number {
    let streak = 0;
  ...
  function processDailyStats(): Array<{ date: string; duration: number }> {
    ...
    return Object.entries(readingStats.stats.dailyStats)
      .map(([date, stats]) => ({
        date,
        duration: Math.round(stats.totalDuration / (1000 * 60)),
      }))
...
<div class="stats-panel" class:open={isOpen}>
  ...
  <div class="panel-content">
    ...
    {#if processedData?.progress !== undefined}
      <div class="progress-section">
        ...
        <div class="progress-bar">
          <div class="progress" style="width: {processedData.progress}%"></div>
        </div>
    ...
    <!-- 导出按钮 -->
    <div class="actions">
      <button class="export-button" on:click={() => dispatch('exportStats')}> 导出报告 </button>
    </div>
  </div>
</div>
