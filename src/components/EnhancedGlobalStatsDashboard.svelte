<script lang="ts">
  ...
  let readingTimeChart: unknown = null;
  let speedDistributionChart: unknown = null;
  let timeSlotChart: unknown = null;
  let weekdayChart: unknown = null;
  ...
  async function loadStats() {
    ...
    try {
      console.log('=== 加载全局统计 ===');
      console.log('statsService:', !!plugin.statsService);

      if (plugin.statsService) {
        // 从新存储加载全局统计
        globalStats = (await plugin.statsService.getGlobalStats()) || null;
        console.log('加载的全局统计:', globalStats);

        if (!globalStats) {
          // 如果没有数据，重新计算
          console.log('没有全局统计数据，开始重新计算...');
          globalStats = (await plugin.statsService.multiFileStorage?.recalculateGlobalStats()) || null;
          console.log('重新计算后的全局统计:', globalStats);
        }

        // 加载所有小说的统计数据（用于图表）
        if (globalStats) {
          console.log('加载所有小说统计...');
          await loadAllNovelStats();
          console.log('小说统计数量:', allNovelStats.length);
          await initCharts();
        } else {
          console.warn('全局统计数据为空');
        }
      } else {
  ...
  async function loadAllNovelStats() {
    try {
      ...
      const statsPromises = novels.map(async (novel) => {
        try {
          return await plugin.statsService?.getNovelStats(novel.id);
        } catch {
          return null;
        }
      });
  ...
  function initSpeedDistributionChart() {
    ...
    const successColor =
      getComputedStyle(document.body).getPropertyValue('--text-success') || '#4caf50';
  ...
  function getAverageSpeed() {
    const speeds = allNovelStats
      .map((s) => s.behaviorStats?.averageReadingSpeed || 0)
      .filter((s) => s > 0);
      ...
      <div class="goal-section">
        ...
        <div class="goal-content">
          <div class="goal-progress-bar">
            <div class="goal-progress-fill" style="width: {yearGoal.progress}%"></div>
          </div>
          <p class="goal-text">
            已完成 <span class="accent">{yearGoal.currentBooks}</span> 本， 目标
            <span class="accent">{yearGoal.targetBooks}</span>
            本 （{yearGoal.progress.toFixed(1)}%）
          </p>
        </div>
      </div>
    {/if}
