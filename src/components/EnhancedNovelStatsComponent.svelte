<script lang="ts">
  ...
  function initDurationChart() {
    ...
    const dailyStats = Object.entries(stats.timeAnalysis.dailyStats)
      .map(([date, stat]) => ({
        date,
...
### 小时分布（Top 3）
${getTopHours(stats.timeAnalysis.hourlyDistribution)
  .map(([hour, duration]) => `- **${hour}:00**: ${formatDuration(duration)}`)
  .join('\n')}

### 星期分布（Top 3）
${getTopWeekdays(stats.timeAnalysis.weekdayDistribution)
  .map(([day, duration]) => `- **${getWeekdayName(day)}**: ${formatDuration(duration)}`)
  .join('\n')}
...
| 章节 | 阅读时长 | 阅读次数 | 平均速度 |
|------|----------|----------|----------|
${getTopChapters(stats.chapterStats)
  .map(
    ([id, chapter]) =>
...
- **笔记最多的章节**: ${stats.notesCorrelation.heatmapChapters
      .slice(0, 3)
      .map((id) => `第${id}章`)
      .join('、')}
...
`;
  ...
  function generateCSVReport(stats: EnhancedNovelStats): string {
    const lines = [
      ...
      ...Object.entries(stats.chapterStats).map(
        ([id, chapter]) => `${id},${chapter.timeSpent},${chapter.readCount},${chapter.averageSpeed}`
      ),
    ];
  ...
  function getTopChapters(chapters: { [key: number]: ChapterStats }): Array<[number, ChapterStats]> {
    return Object.entries(chapters)
      .map(([id, chapter]) => [Number(id), chapter] as [number, ChapterStats])
  ...
  {:else}
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card">
        ...
        <p class="stat-sub">
          {stats.progressTracking.completedChapters.length}/{stats.novel.totalChapters || 0} 章
        </p>
      </div>
      <div class="stat-card">
        <div class="stat-icon">🚀</div>
