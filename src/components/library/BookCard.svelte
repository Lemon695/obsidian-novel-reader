...
<div class="book-card">
  ...
  <div class="book-info">
      ...
      <div class="book-tags">
        {#each novel.tags as tagId}
          {#if tags.find((t) => t.id === tagId)}
            <span class="tag" style={getTagStyle(tagId)} on:click={(e) => handleTagClick(tagId, e)}>
    ...
    <!-- 菜单按钮 -->
    <button class="menu-button" on:click={handleMenuClick}> ••• </button>

    <!-- 菜单面板 -->
    {#if showMenu && menuPosition}
      <div class="menu-panel" style="top: {menuPosition.top}px; left: {menuPosition.left}px;">
        <div class="menu-item" on:click={() => manageTags()}>管理标签</div>
        <div class="menu-section">
          移动到书架
          <div class="shelf-submenu">
            <div class="menu-item" on:click={() => moveToShelf('reading')}>在读</div>
            <div class="menu-item" on:click={() => moveToShelf('toread')}>待读</div>
            <div class="menu-item" on:click={() => moveToShelf('finished')}>已读</div>
            <div class="menu-item" on:click={() => moveToShelf('archived')}>归档</div>
          </div>
        </div>
      </div>
    {/if}
