<script lang="ts">
  ...
  async function loadStats() {
    try {
      ...
      // 计算基础统计
      statsData.booksRead = novels.filter(
        (novel: Novel) => novel.progress !== undefined && novel.progress === 100
      ).length;

      statsData.booksInProgress = novels.filter(
        (novel: Novel) => novel.progress !== undefined && novel.progress > 0 && novel.progress < 100
      ).length;
      ...
      statsData.averageCompletionRate =
      ...
      statsData.readingGoal.current = novels.filter(
        (novel: Novel) =>
          novel.progress === 100 &&
          novel.lastRead &&
