...
export class StatsValidator {
    ...
    static verifyChecksum(data: any, expectedChecksum: string): boolean {
        // 临时移除checksum字段进行验证
        const {basicStats, ...restData} = data;
        const {dataChecksum, ...restBasicStats} = basicStats || {};
    ...
    static validateNovelStats(stats: EnhancedNovelStats): IntegrityCheckResult {
        ...
        if (stats.progressTracking) {
            if (stats.progressTracking.currentProgress < 0 || stats.progressTracking.currentProgress > 100) {
                errors.push('currentProgress out of range (0-100)');
            }

            // 检查进度历史是否单调递增（允许小幅波动）
            if (stats.progressTracking.progressHistory && stats.progressTracking.progressHistory.length > 1) {
                for (let i = 1; i < stats.progressTracking.progressHistory.length; i++) {
                    ...
                    if (curr.progress < prev.progress - 5) { // 允许5%的倒退
                        warnings.push(`Progress regression detected at ${new Date(curr.timestamp).toISOString()}`);
                    }
                }
            }
        }
    ...
    static validateGlobalStats(stats: EnhancedGlobalStats): IntegrityCheckResult {
        ...
        if (!stats.library) {
            ...
            // 检查书籍统计一致性
            const totalByStatus = Object.values(stats.library.booksByStatus || {}).reduce((a, b) => a + b, 0);
    ...
    static validateIndexFile(index: StatsIndexFile): IntegrityCheckResult {
        ...
        if (!index.novelIndex) {
            ...
            if (actualCount !== index.totalNovels) {
                warnings.push(`novelIndex count (${actualCount}) does not match totalNovels (${index.totalNovels})`);
            }
        }
    ...
    static repairNovelStats(stats: Partial<EnhancedNovelStats>): EnhancedNovelStats | null {
        try {
            ...
            // 验证修复后的数据
            const validation = this.validateNovelStats(repaired);
