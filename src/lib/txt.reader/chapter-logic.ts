...
export function parseChapters(text: string, novel: Novel): ChapterProgress[] {
  ...
  return allMatches.map((match, index) => {
    ...
    // 二级标题判断（节、小节、部分等）
    if (
      title.includes('节') && !title.includes('章') || // 第X节
      lowerTitle.includes('section') ||
      title.match(/^[0-9]+\.[0-9]+/) || // 1.1, 2.3 等
      title.match(/^[（(][0-9]+[)）]/) // (1), (2) 等
    ) {
      level = 1;
    }
...
export function handleChapterChange(
  ...
  if (chapter && novel) {
    historyService
      .addHistory(novel.id, chapter.id, chapter.title)
      .then(() => historyService.getHistory(novel.id))
      .then((history: unknown[]) => onHistoryUpdate(history));
  }
}
